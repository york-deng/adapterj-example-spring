package com.adapterj.example.springmvc.simple;
...
import com.adapterj.ext.spring.InitController;
import com.adapterj.ext.spring.SimpleHttpParametersResolver;
...
@RestController
public class SimpleController extends InitController {

    private static final boolean DEBUG = Debugger.DEBUG;
    private static final String TAG = SimpleController.class.getName();

    private final String _propertiesFile = "/adpj.properties";
    private Registry _registry = null;
    
    private final String _templateFile1 = "/templates/index.html";
    private String _acceleratorClass1 = null;
    private Document _document1 = null;
    private String _md51 = null;
    
    private final String _templateFile2 = "/templates/simplelist.html";
    private String _acceleratorClass2 = null;
    private Document _document2 = null;
    private String _md52 = null;
    
    private final String _templateFile3 = "/templates/simpleform.html";
    ...
    public SimpleController() {
        try {
            final String dir = SimpleController.class.getClassLoader().getResource("").getPath();
            if (DEBUG) {
    ...
    @ModelAttribute
    void parameters(final HttpServletRequest request) { 
        _baseURI = getBaseURI(request);
        ...
        final Source parameter = resolver.getParameter(request); // new Source(); 
        if (DEBUG) {
            StackTraceElement t = (new Throwable()).getStackTrace()[0];
    ...
    @GetMapping("/simple/parameter")
    @ResponseBody
    public String parameter(final @RequestAttribute("parameter") Object parameter, final HttpServletResponse response) {
        response.setContentType("text/json");
        response.setCharacterEncoding("UTF-8");
        return "Got 'parameter' request attribute value '" + parameter + "'";
    }

    @GetMapping("/simple/list/html")
    @ResponseBody
    public String list(final HttpServletResponse response) {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            final StringBuffer s = new StringBuffer(_baseURI);
            ...
            while(iter.hasNext()) {
                ...
                s.delete(_baseURI.length(), s.length());
        ...
        */
        ...
        if (DEBUG) { // DEBUG
            StackTraceElement t = (new Throwable()).getStackTrace()[0];
            String format = "(%s:%d) %s: list: cost is %d";
    ...
    @GetMapping("/simple/form/html")
    @ResponseBody
    public String form(final @RequestAttribute("parameter") Object object, final HttpServletResponse response) {
        ...
        */
        //final SimpleHttpParametersResolver<Source> resolver = new SimpleHttpParametersResolver<Source>(_registry);
        final Source parameter = (Source) object; // resolver.getParameter(context.request());
        if (DEBUG) {
            StackTraceElement t = (new Throwable()).getStackTrace()[0];
            String format = "(%s:%d) %s: parameter is %s";
            Log.i(TAG, String.format(format, t.getFileName(), t.getLineNumber(), t.getMethodName(), parameter));
        ...
        */
        ...
        if (DEBUG) {
            StackTraceElement t = (new Throwable()).getStackTrace()[0];
            String format = "(%s:%d) %s: form: end is %s, cost is %d";
